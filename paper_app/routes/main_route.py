...
def compare_index():
    ...
    model_list = [{"model_name":model.model_name,"modle_pkl":model.modle_pkl} for model in raw_model_list]
    modelname, submit_x = "[ ____ ]","[ ____ ]"
    ...
    if request.method == "POST":
        ...
        for model in model_list:
            if model['model_name'] == modelname:
                submit_x = model.model_pkl['submit']['X'][x_row]
...
def user_index():
    ...
    raw_model_list = Trained.query.all()
    model_list = [{"id":model.id, "model_name":model.model_name,"modle_pkl":model.modle_pkl} for model in raw_model_list]
